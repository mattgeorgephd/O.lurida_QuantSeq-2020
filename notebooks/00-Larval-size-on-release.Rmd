...
```{r, echo=FALSE}
# FIGURE FOR PAPER 
print(max.mean <- ggplot(subset(larvae.size.mean, Population!="Oyster Bay C2"), aes(x=Temperature:pH, y=MaxFeret.mean)) + 
...
print(max.mean <- ggplot(data=subset(larvae.size.mean, Population!="Oyster Bay C2"), aes(x=Temperature:pH, y=MaxFeret.mean)) + 
          geom_boxplot() + geom_jitter(width=0.18, size=2.5, aes(color=Population), alpha=0.75) + 
          theme_bw() + labs(title="Larval shell width, by parental treatment & cohort", y=("Shell width (um)"), x="Treatment") + ylim(c(164.5,195)) + 
          theme(text = element_text(size = 10), axis.title.x = element_blank()) + 
          scale_color_manual(values=c("#4daf4a","#377eb8","#984ea3","#e41a1c"), name="Cohort") + 
          #scale_size_date(name = "Date Released") + 
          scale_x_discrete(labels=c("6:ambient" = "6°C\nAmbient\npCO2", "6:low" = "6°C\nHigh\npCO2", 
                                    "10:ambient" = "10°C\nAmbient\npCO2", "10:low" = "10°C\nHigh\npCO2")) +
    guides(colour = guide_legend(override.aes = list(size=4.5)))) + facet_wrap(~Population)
...
summary(lm(MaxFeret.mean ~ Temperature+pH, data=larvae.size.mean)) # 
TukeyHSD(aov(MaxFeret.mean ~ Temperature*pH, data=larvae.size.mean), type=2) #both sign. 

# Sign. w/o OB2? 
anova(lm(MaxFeret.mean ~ Temperature + pH + Temperature:pH, data=subset(larvae.size.mean, Population!="Oyster Bay C2"))) #no interaction. Assess using type II sums of squares, where I look at effects of pH and temperature after controlling for the other factor 
Anova(lm(MaxFeret.mean ~ Temperature*pH, data=subset(larvae.size.mean, Population!="Oyster Bay C2")), type=2) #both sign. 
summary(lm(MaxFeret.mean ~ Temperature+pH, data=subset(larvae.size.mean, Population!="Oyster Bay C2"))) # 
```

Test for sign. differences among parental treatments within population 

```{r}
Anova(lm(MaxFeret.mean ~ Population*Temperature*pH, data=larvae.size.mean), type=2) 
Anova(aov(MaxFeret.mean ~ Temperature+pH+Population:Temperature:pH, data=larvae.size.mean), type=2)  
summary(aov(MaxFeret.mean ~ Temperature+pH+Population:Temperature:pH, data=larvae.size.mean), type=2)  
TukeyHSD(aov(MaxFeret.mean ~ Temperature+pH+Population:Temperature:pH, data=larvae.size.mean), type=2) 

TukeyHSD(aov(MaxFeret.mean ~ Temperature+pH+Population:Temperature:pH, data=larvae.size.mean), type=2)$`Temperature:pH:Population` %>% as.data.frame() %>% filter(`p adj`<0.05)
```
...
print("Oyster Bay C1")
summary(aov(MaxFeret.mean ~ Temperature*pH, data=subset(larvae.size.mean, Population=="Oyster Bay C1")))  #temp & pH significant difference 
