...
pheatmap(assay(vsd.adult[degs.adult.pops,]), cluster_rows=T, show_rownames=FALSE, show_colnames=FALSE,
         cluster_cols=T, annotation_col=vsd.adult.df[1], scale="row", main = "DEGs among cohorts, Adults",
...
```{r}
# generate heatmap of population DEGs  
vsd.adult.ambient.df <- as.data.frame(colData(vsd.adult.ambient)[,c("population", "pCO2.parent")])

pheatmap(assay(vsd.adult.ambient[degs.adult.pops.amb,]), cluster_rows=T, show_rownames=FALSE, show_colnames=FALSE,
         cluster_cols=T, annotation_col=vsd.adult.ambient.df[1], scale="row", main = "DEGs among cohorts, Adults (ambient pCO2 only)",
         annotation_colors=list(population=c(`Dabob Bay` = "#4daf4a", `Fidalgo Bay` = "#377eb8", `Oyster Bay C1` = "#984ea3")))
```
