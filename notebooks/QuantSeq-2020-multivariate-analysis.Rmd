...
```{r}
counts <- data.frame(read.table("../qc-processing/featureCounts/test-out_featurecounts.Rmatrix.txt", header = T, stringsAsFactors = F, fill = FALSE)) %>%
...
print(paste("# of adult ctenidia genes for analysis:", ncol(counts.ts.adult.s), sep=" ")) # 76879 in adult samples 
print(paste("# of juvenile whole-body genes for analysis:", ncol(counts.ts.juvenile.s), sep=" ")) # 56852 in juvenile samples 
print(paste("# of larval whole body (pooed) genes for analysis:", ncol(counts.ts.larvae.s), sep=" ")) # 67916 in larval samples 
...
ncol(counts.ts.adult.s) - ncol(counts.ts.adult.sv) #number of contigs/genes dropped due to low variance .. 4255 

counts.ts.juvenile.sv <- drop.var(counts.ts.juvenile.s, min.cv=5) 
ncol(counts.ts.juvenile.s) - ncol(counts.ts.juvenile.sv) #number of contigs/genes dropped due to low variance .. 3140 
