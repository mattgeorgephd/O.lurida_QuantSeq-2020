...
larval.samples <- key.filtered %>% filter(stage=="larvae") %>% select(sample) %>% unlist() %>% as.vector() %>% as.character()

ggplotly(
ggplot(data = counts.filtered %>%
         select(larval.samples) %>%
         colSums() %>% data.frame() %>%
...
dds.larvae <- DESeqDataSetFromMatrix(countData = counts.tsk.larvae[,grepl("OLUR", colnames(counts.tsk.larvae))] %>% t(),
                              colData = counts.tsk.larvae[,c("population", "temp.parent", "pCO2.parent", "week.collected", "prep.batch", "larval.day.collected")],
                              design = ~ population + temp.parent + pCO2.parent)
```
...
plotPCA(vsd.larvae, intgroup="population") + 
  ...
  geom_point(aes(size=vsd.larvae$larval.day.collected, text=colnames(vsd.larvae))) + theme_minimal(), tooltip = "text")

# PCA with points color coded by parental temperature treatment
ggplotly(
plotPCA(vsd.larvae, intgroup="temp.parent") + 
           ggtitle("PCA by population (var-stabilizing transformed)") + 
  geom_point(size=3) + theme_minimal(), tooltip = "text")
...
plotPCA(vsd.larvae, intgroup="pCO2.parent") + 
  ...
  geom_point(size=3, aes(text=colnames(vsd.larvae))) + theme_minimal(), tooltip = "text")
...
ggplotly(
  plotPCA(vsd.larvae, intgroup=c("population","pCO2.parent")) + 
    ...
    scale_color_manual(values = c("#F8766D","#00BFC4","#F8766D","#00BFC4","#F8766D","#00BFC4","#F8766D","#00BFC4")), tooltip = "text")
