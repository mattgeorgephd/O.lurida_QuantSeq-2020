...
ggplot(data = data.frame(colSums(counts.filtered)) %>% 
           ...
           geom_bar(aes(x=sample, y=count.total), stat = "identity") + ggtitle("Total count by sample, juvenile ctenidia tissue") + 
...
#### If warranted, check out sample ke  
```{r}
key %>% filter(stage=="juvenile") %>% select(sample, population, temp.parent, pCO2.parent)
```


### Extract juvenile samples only
```{r}
counts.t.juv <- as.data.frame(counts.t[grepl(".juvenile", rownames(counts.t)), ])
...
pheatmap(data.matrix(counts.tsk.juv %>% dplyr::select(starts_with("OLUR"))), Rowv=NA, Colv=NA, na.rm = TRUE, xlab = NA, 
         ...
         filename = "../results/heatmap-juvenile-counts.pdf")
```

Resulting heat map is located in results/ directory: 
![heatmap-juvenile-counts.pdf](https://github.com/laurahspencer/O.lurida_QuantSeq-2020/blob/master/results/heatmap-juvenile-counts.pdf)
